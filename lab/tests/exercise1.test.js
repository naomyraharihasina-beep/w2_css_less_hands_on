...
describe('TASK 1D: Use Variables', () => {
  ...
  test('NO hardcoded colors', () => {
    const withoutComments = lessContent
      .replace(/\/\*[\s\S]*?\*\//g, '')
      // Remove variable definition lines so only usages are checked
      .replace(/@brand-color\s*:\s*#16a085.*\n?/, '')
      .replace(/@brand-color-dark\s*:\s*#138d75.*\n?/, '');
